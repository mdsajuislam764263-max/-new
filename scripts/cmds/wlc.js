...
// Fetches Facebook profile picture using a public Graph API token
...
export default {
  ...
  // In some Messenger frameworks (GoatBot/Mirai), this might need to be named `onEvent` 
  onChat: async (client, message) => {
    // Check if the event is a new member joining (Messenger specific logotype)
    if (message.logMessageType !== "log:subscribe") return;
    if (!message.logMessageData || !message.logMessageData.addedParticipants) return;

    const threadID = message.threadID;
    const adderId = message.author;
    ...
    try {
      // Fetch thread & adder details
      const threadInfo = await client.getThreadInfo(threadID);
      groupName = threadInfo.threadName || groupName;
      memberCount = threadInfo.participantIDs?.length || 1;

      if (adderId) {
        const userInfo = await client.getUserInfo(adderId);
        if (userInfo && userInfo[adderId]) {
          adderName = userInfo[adderId].name;
        }
      }

      // Fetch group image if available in Messenger thread
      if (threadInfo.imageSrc) {
        const { loadImage } = await getCanvas();
        const res = await axios.get(threadInfo.imageSrc, { responseType: "arraybuffer" });
        groupPhoto = await loadImage(Buffer.from(res.data));
      }
    } catch {}

    for (const newMember of message.logMessageData.addedParticipants) {
      const userId = newMember.userFbId;
      const userName = newMember.fullName;

      // Ignore if the bot itself was added (Optional)
      if (userId === client.getCurrentUserID?.()) continue;

      try {
        const userPhoto = await getUserPhoto(userId);
        const adderPhoto = adderId ? await getUserPhoto(adderId) : null;

        const imgBuffer = await createWelcomeCard(groupPhoto, userPhoto, adderPhoto, userName, memberCount, groupName, adderName);
        
        // Save temporary canvas image
        const temp = path.join(__dirname, "../data", `welcome_${Date.now()}_${userId}.png`);
        ...
        await client.sendMessage(`🌸 Welcome ${userName}! 🌸\nEnjoy your stay!`, threadID);
      }
    }
  },

  execute: async (client, message, args) => {
    await client.sendMessage("✅ Auto-welcome is active. Add a member to test it.", message.threadID);
  }
};
